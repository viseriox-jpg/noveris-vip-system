...
final class KitEditorMenu extends ChestMenu {
    ...
    private final List<ItemStack> temporary = new ArrayList<>(), permanent = new ArrayList<>();
    private final Map<String, Integer> originalCounts = new HashMap<>();
    private final HolderLookup.Provider registries;
    private int page;
    private boolean temporaryMode = true, closedByButton;

    KitEditorMenu(int id, Inventory inventory, SimpleContainer display, VipService service,
                  String kitName, String plan, VipStore.Kit existing) {
        ...
        if (existing != null) for (VipStore.KitItem item : existing.items) {
            ItemStack stack = VipStore.decode(item.encodedStack(), registries).copy();
            (item.temporary() ? temporary : permanent).add(stack);
            originalCounts.merge((item.temporary() ? "T:" : "P:") + item.encodedStack(), 1, Integer::sum);
        }
        showPage();
    }
    ...
    private void installControls() {
        for (int slot = 45; slot < 54; slot++) {
            ...
            pane.set(DataComponents.CUSTOM_NAME, Component.literal("Editor de kit VIP").withStyle(ChatFormatting.GRAY));
            display.setItem(slot, pane);
        }
        control(PREVIOUS_SLOT, Items.ARROW, "← PÁGINA ANTERIOR", ChatFormatting.AQUA);
        control(PAGE_SLOT, Items.PAPER, "PÁGINA " + (page + 1) + " • " + active().size() + " pilhas", ChatFormatting.YELLOW);
        ...
        control(SAVE_SLOT, Items.EMERALD_BLOCK, "SALVAR KIT", ChatFormatting.GREEN);
    ...
    @Override public void clicked(int slotId, int button, ClickType clickType, Player player) {
        ...
        if (slotId == SAVE_SLOT) {
            capturePage();
            service.saveKit(staff, kitName, plan, temporary, permanent);
            closedByButton = true;
            returnAll(staff);
        ...
        if (slotId == CANCEL_SLOT) {
            ...
            returnAll(staff);
            staff.closeContainer();
            staff.sendSystemMessage(Component.literal("Edição cancelada.").withStyle(ChatFormatting.RED));
        }
    }
